...
const CourseCard = ({ course }) => {
    ...
    const fetchlessons = async () => {
        await axios.get(`${import.meta.env.VITE_DEV_URL}study/getlesson?moduleId=${id}`).then((res) => {
            console.log(res.data);
            setlessons(res.data.data);
        });
    };
    const fetchmodule = async () => {
        await axios.get(`${import.meta.env.VITE_DEV_URL}study/getmodule?moduleId=${id}`).then((res) => {
            console.log(res.data);
    ...
    return (
        <>
            <h1 className="mb-6 ml-10 text-3xl font-bold dark:text-blue-500">{currentmodule.title}</h1>
            <div className="flex justify-evenly items-center flex-wrap">
            {lessons.map((lesson, idx) => (
                <div key={idx} className="cursor-pointer course-card group relative m-8 flex h-40 w-40 transform items-center justify-center overflow-hidden rounded-2xl bg-white p-4 text-xl font-bold shadow-lg transition-transform hover:scale-105 dark:bg-slate-950">
                    <div className="absolute inset-0 flex items-center justify-center text-3xl transition-opacity group-hover:opacity-0 dark:text-blue-500">
                        {lesson.title}
                    </div>
                    <img
                        src={lesson.signImgUrl}
                        alt={lesson.title}
                        className="absolute inset-0 h-full w-full rounded-md object-cover opacity-0 transition-opacity group-hover:opacity-100"
                    />
                </div>
            ))}
            </div>
